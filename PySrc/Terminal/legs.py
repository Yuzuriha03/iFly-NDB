...
from Terminal.list import list_generate

warnings.filterwarnings('ignore')

def get_revision_code_from_config(db_path=r"C:\ProgramData\Fenix\Navdata\nd.db3"):
...
def process_files(root, files, permanent_path, supplemental_path_base):
    icao_prefixes = ('OPGT', 'VHHX', 'ZB', 'ZG', 'ZH', 'ZJ', 'ZL', 'ZP', 'ZS', 'ZU', 'ZW', 'ZY')
...
def terminals(conn, navdata_path, start_terminal_id, end_terminal_id):
    ...
    # 生成FMC_Ident.txt
    # 优先读取 Fenix 默认路径 nd.db3 的 config 表。
    matched_rev_code = get_revision_code_from_config()
